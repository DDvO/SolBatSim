...
# * <base_name><power_name>_<date>.csv  load and optional PV input per second
...
if ($addr eq "-") {
    ...
    while(<>) {
        ...
        if ($#elems == 5) {
            ...
            log_warn("load $load is not consistent with ".sprintf("%7.2f", $sum)
                     ." = sum of phases ".
...
sub do_before_day {
   ...
   "powerC [W],pfC,currentC [A],voltageC [V],totalC [Wh],total_returnedC [Wh]\n"
    ...
    print $PW "time [$tz],load [W], PV power [W],".
        ...
        if -z $powers; # on empty pv output CSV file, add header
}
